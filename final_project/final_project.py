...
shiftss[0]=124
shiftss[last_atom]=123
len(shiftss)
for i in range(len(shiftss)):
    shiftss[i]=float(shiftss[i])
def Total_atoms(variables_list):
...
def converted_variables(filename):
    ...
    chem_shifts_var=[]
    total_atoms=Total_atoms(old_variables)
    number_atoms=len(total_atoms)
    for atoms  in Total_atoms(old_variables):
...
def atoms_distances(array_of_comb_numb):
    ...
    for i in range(len(array_of_comb_numb)):
        if (array_of_comb_numb[i][2]>1.0):   # only atoms that are closer than 5.5 Angstroms will be considered
            next
        else:
...
XX_means=np.array(x_H)
YY_means=np.array(y_H)
intens=np.array(intense)
x_atoms=np.array(shiftss)
X_2d=[]
Y_2d=[]
...
X_2d=np.array(X_2d)
Y_2d=np.array(Y_2d)
Y_1d=np.arange(N_atoms)
sigma=0.0001
def gaus(x, y, intensity):
    ZZZ=np.zeros((N_atoms,N_atoms))
    for i in range(len(X_points)):
        for k in range(len(X_points)):
            for l in range(len(x)):
                ZZZ[i][k]+=intensity[l]*math.exp(-((X_points[k]-x[l])**2+(X_points[i]-y[l])**2)/(2*sigma**2))
                
    return ZZZ
ZZ=gaus(XX_means, YY_means, intense)
ZZ=np.array(ZZ)
fig, ax = plt.subplots()
len(shiftss), len(intens), len(x_H)
sc=ax.scatter(x_H, y_H, intense)
ax.grid()
